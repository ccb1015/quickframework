...
%>
<style>
	#pagediv<%=randId%>{font-familay:宋体;font-size:12px;}
	#pagediv<%=randId%> button{font-color:#666666;height:26px;border: #C9C9C9  1px solid;background-color: #fff;
		margin-left:2px;margin-right:2px;padding:2px;width:48px;
		border-radius:3px;font-size:11px;cursor:pointer;
	}
	#pagediv<%=randId%> button[disabled]{color: inherit;border: #fff  1px solid;background-color: #fff;cursor:default;}
	#pagediv<%=randId%> input[type="text"]{height: 26px;width: 26px;padding:0 2px;margin: 0 2px;border: #D3D3D3 thin solid;border-radius: 3px;vertical-align: middle;}
</style>
 ...
 <script type="text/javascript">
	...
	function turnToPage<%=randId%>(no,pageSize){ 
		...
		if(no<1){no=1;} 
		 window._pageInfo_<%=randId%>['pageInfo.currentPage']= no; 
		 window._pageInfo_<%=randId%>['currentPage']= no; 
		 
		 window._pageInfo_<%=randId%>['pageInfo.pageSize']=pageSize?parseInt(pageSize,10):parseInt(window._pageInfo_<%=randId%>.pageSize,10);
		 if(pageSize){
			 window._pageInfo_<%=randId%>['pageSize']=parseInt(pageSize,10);
		 } 
		 window._pageInfo_<%=randId%>['pageInfo.totalCount']=parseInt( window._pageInfo_<%=randId%>.totalCount,10);
	...
	function buildPage<%=randId%>(){
		if(<%=type%>==1){
			if(window._pageInfo_<%=randId%>.totalCount<=0){
				var htmlStr="<button disabled class='disabled'>首页</button>\
				<button disabled class='disabled'>上一页</button>\
				<button disabled class='disabled'>下一页</button>\
				<button disabled  class='disabled'>尾页</button>\
				...
				&nbsp;&nbsp;\
				<button style='background-color:#F3791E;color:#fff;width:30px;border: 0;'  disabled class='disabled'>go</button>\
				&nbsp;&nbsp;\
				【 第 0/0页 , 共 0条记录 】</span>";
				
				jQuery("#pagediv<%=randId%>").html(htmlStr);
			}else{  
